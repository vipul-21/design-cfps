...
Since policy revision numbers from Cilium are reset when the local Cilium instance restarts, we need to unconditionally send the policy revision number to the SDP instance on agent restart.
...
* Provides a freedom to allows any delegated dns proxy to be plugged in.
...
* Cilium agent can keep serving the DNS queries through in built DNS proxy. In case, the inbult dns proxy is disabled, the SDP needs an upgrade path. This can be achieved using the `maxsurge` on the sdp. This can be future milestone.
...
| Down | Down | Running | Works | Datapath should work too if already configured. |
